...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isAdmin }) => {
  ...
  const navItems = [
    ...
    { id: 'user_management', label: 'Usuários', icon: <UserIcon />, adminOnly: true },
  ];
      ...
      <aside className={`relative bg-white dark:bg-[#222b39] text-slate-700 dark:text-white flex flex-col transition-all duration-300 ease-in-out sticky top-0 h-screen ${isCollapsed ? 'w-[72px]' : 'w-64'}`}>
              ...
              <li key={item.id}>
                <button
                  ...
                  className={`w-full flex items-center gap-3 rounded-lg text-left p-3 my-1 transition-colors ${
                    activeView === item.id 
                    ? 'bg-orange-100 dark:bg-[#FD7F08] text-orange-600 dark:text-white font-semibold shadow-md shadow-orange-500/10 dark:shadow-none' 
                    : 'hover:bg-slate-100 dark:hover:bg-white/10 text-slate-500 dark:text-slate-300'
        ...
        <div className="p-2 border-t border-slate-200 dark:border-white/10 shrink-0">
          <button
              ...
              title="Editar Perfil"
              className={`w-full flex items-center p-2 rounded-lg gap-3 text-left hover:bg-slate-100 dark:hover:bg-white/10 transition-colors ${isCollapsed ? 'justify-center' : ''}`}
          >
              {user?.photoURL ? (
                  <img src={user.photoURL} alt="Foto de perfil" className="w-8 h-8 rounded-full object-cover shrink-0" />
              ) : (
                  <UserIcon className="w-8 h-8 shrink-0 p-1 bg-slate-200 dark:bg-slate-600 rounded-full" />
              )}
          ...
          <button
            onClick={logout}
            title="Sair"
            className={`w-full flex items-center gap-3 rounded-lg text-left p-3 my-1 transition-colors bg-slate-100/50 dark:bg-white/5 hover:bg-red-500/10 dark:hover:bg-red-500/20 text-red-500 dark:text-red-300 hover:text-red-600 dark:hover:text-red-200 ${isCollapsed ? 'justify-center' : ''}`}
          >
            <LogoutIcon />
        ...
        <button 
          ...
          className="absolute top-1/2 -translate-y-1/2 right-0 translate-x-1/2 bg-white dark:bg-[#222b39] border-2 border-slate-300 dark:border-slate-600 rounded-full w-7 h-7 grid place-items-center text-slate-400 hover:text-white hover:bg-[#FD7F08] hover:border-[#FD7F08] transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-[#222b39] focus:ring-[#FD7F08] z-10"
