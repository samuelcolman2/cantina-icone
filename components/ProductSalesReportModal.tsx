  ...
  const renderContent = () => {
    ...
    if (filterType === 'today') {
      if (todaySales.length === 0) {
        return <p className="text-center text-slate-500 p-8">Nenhuma venda registrada hoje para este produto.</p>;
      }
      return (
        <ul className="space-y-3">
          {todaySales.map(entry => (
            <li key={entry.id} className="flex items-center justify-between p-2.5 rounded-lg bg-slate-50/80 text-sm">
              <span className="font-medium text-slate-600">Venda realizada às:</span>
              <span className="text-slate-800 font-semibold">{formatDate(entry.timestamp)}</span>
            </li>
          ))}
        </ul>
      );
    ...
    if (filterType === 'history') {
      ...
      if (uniqueDates.length === 0) {
        return <p className="text-center text-slate-500 p-8">Nenhum histórico de vendas para este produto.</p>;
      }
      ...
      return (
        <ul className="space-y-3">
          {uniqueDates.map(date => (
            <li key={date}>
              <button onClick={() => setSelectedDate(date)} className="w-full text-left flex items-center justify-between p-2.5 rounded-lg bg-slate-50/80 hover:bg-slate-100 transition-colors text-sm">
                <span className="font-medium text-slate-700">Vendas do dia {date}</span>
                <span className="text-slate-500 font-semibold bg-slate-200 text-xs py-0.5 px-2 rounded-full">{salesByDate[date].length}</span>
              </button>
            </li>
          ))}
        </ul>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 sm:p-8 relative flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="text-2xl font-bold text-slate-800 mb-1">Relatório de Vendas</h2>
        <p className="text-slate-500 mb-4">{product.name}</p>

        <div className="border-b border-slate-200">
          <div className="flex -mb-px">
            <button
              onClick={() => { setFilterType('today'); setSelectedDate(null); }}
              className={`py-3 px-4 text-sm font-semibold border-b-2 transition-colors ${filterType === 'today' ? 'border-orange-500 text-orange-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}
            >
              Vendas de Hoje
            </button>
            <button
              onClick={() => { setFilterType('history'); setSelectedDate(null); }}
              className={`py-3 px-4 text-sm font-semibold border-b-2 transition-colors ${filterType === 'history' ? 'border-orange-500 text-orange-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}
            >
              Histórico
            </button>
          </div>
